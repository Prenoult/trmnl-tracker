...
export function buildChartModel(history) {
  ...
  const projectionDate = estimate?.date ?? null;
  const startDate = parseDay(history[0].date);
  const lastDate = parseDay(history[lastIndex].date);
  const span = (projectionDate ?? lastDate) - startDate || 1;

  const positions = history.map((p) => p.position);
  const tickValues = niceTicks(projectionDate ? 0 : Math.min(...positions), Math.max(...positions));
  ...
  return {
    ...
    projection: projectionDate
      ? { date: projectionDate, x: xAt(projectionDate), y: y(0) }
      : null,
    ...
    dateMarks: [
      ...
      ...(xAt(lastDate) > left + plotW * 0.25 && xAt(lastDate) < left + plotW * 0.75
        ...
        : []),
      ...(projectionDate
        ? [{ date: projectionDate, at: xAt(projectionDate), anchor: "end" }]
