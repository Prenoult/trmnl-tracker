...
// shipped: the order has a known outcome now, so a dashed line projecting
// where it *would* have shipped is no longer a forecast — it is stale, and
// showing it next to the real result reads as the app not having noticed. Only
// the projection is cut; the position/total curves and the gains strip below
// stay exactly as recorded, because how the wait actually went never stops
// being true.
export function buildChartModel(history, { shipped = false } = {}) {
  ...
  const estimate = shipped ? null : shippingEstimate(history);
