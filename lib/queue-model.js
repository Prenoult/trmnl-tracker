...
export const QUEUE = {
  ...
  trailY: 96,
  labelY: 108,
  // Half the room the trail's label needs, so clamping its centre keeps the whole
  // string inside the lane. It carries the direction ("303 places gagnées"), which
  // is why it is wider than the figure alone — and why the caption below the
  // drawing no longer has to spend a line saying it in prose.
  labelPad: 72,
...
//
// shipped: by FIFO, an order that has shipped has, by definition, nothing left
// ahead of it — every order that was ever ahead shipped first. So rather than
// draw the marker back where it was last seen (which reads as just another
// snapshot, not an ending), shipped mode draws it at the door itself: `ahead`
// goes to 0, every tick reads as ground already covered instead of a wait still
// owed, and the trail — the whole journey, not just the last leg — runs all the
// way there. `position`/`total`/`behind` stay the last real recorded facts:
// only where the marker is *drawn* changes, not what actually happened.
export function buildQueueModel(latest, first = null, { shipped = false, gained = null } = {}) {
  ...
  const x = shipped ? left : xAt(position);
  const { count, step, width } = tickLayout(laneW, total);
  const markerFraction = shipped ? 0 : fractionOf(position, total);
  ...
  const ticks = Array.from({ length: count }, (_, i) => ({
    ...
    width,
    ahead: shipped ? false : (i + 0.5) / count < markerFraction,
  }));
  ...
  return {
    ...
    // total − 1, the queue minus the order itself. Once shipped, "ahead" is
    // truthfully 0 — FIFO means every order that was ever ahead has shipped
    // too — while `behind` keeps naming the last recorded count: not a live
    // figure any more, but still what the queue looked like the day it left.
    ahead: shipped ? 0 : position - 1,
    ...
    trail: !hasTrail
      ...
      : {
          ...
          // In shipped mode the caller passes the real whole-series figure
          // (journeySummary's), since the marker no longer sits at `position`
          // and startRank − position would understate the ground actually
          // covered on the unmeasured final stretch to the door.
          gained: gained ?? startRank - position,
          labelX: clamp((startX + x) / 2, left + QUEUE.labelPad, w - right - QUEUE.labelPad),
          labelY: QUEUE.labelY,
        },
  };
}
