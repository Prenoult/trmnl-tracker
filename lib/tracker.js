...
// The terminal state parseQueueStatus can never see a position for: once shipped,
// trmnl.com stops rendering "You are #N in a queue" at all, and the retry loop
// would otherwise burn its whole attempt budget before failing with a message
// that was, until this existed, just a guess in the error text.
const SHIPPED_RE = /order has been shipped/i;

export function isShipped(html) {
  return SHIPPED_RE.test(html);
}
...
export async function fetchQueueStatus(
  ...
  for (let attempt = 0; attempt < attempts; attempt++) {
    ...
    if (isShipped(lastBody)) return { shipped: true };
    const status = parseQueueStatus(lastBody);
    if (status) return status;
    await sleep(STATUS_RETRY_MS);
  }
  // Shipped is handled above; reaching here means the frame settled on something
  // isShipped and parseQueueStatus both fail to recognise — a reworded page, most
  // likely. Printing what it actually got back is what turns the next failure
  // report into a diagnosis instead of a guess.
  throw new Error(
    `Status frame never returned a queue position or a shipped notice. ` +
